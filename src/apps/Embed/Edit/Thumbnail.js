...
module.exports = {
    name: 'thumbnail',
    description: 'Chỉnh sửa thumbnail của embed',
    cooldown: 3,
    permission: {
    ...
    options: [
        ...
        {
            name: 'url',
            description: 'Link ảnh thumbnail cho embed',
            type: 'String',
            required: false
        }
    ],
    run: async (client, interaction, user, prefix, message, lang) => {
            ...
            return interaction.editReply({
                embeds: [
                    client.embed()
                        ...
                        .setAuthor(embed.embed.author ? { name: client.variable(embed.embed.author.text, interaction), iconURL: await client.variable(embed.embed.author.icon, interaction) } : null)
            ...
            return interaction.editReply({
                embeds: [
                    client
                        ...
                        .setThumbnail(null)
                        .setImage(embed.embed ? await variable(embed.embed.image, interaction) : null)
                ]
            });
        }
        ...
        await interaction.editReply({
            content: `${client.e.done} | Thumbnail của embed \`${name}\` đã được cập nhật.`,
            embeds: [
                client
                    .embed()
                    .setColor(embed.embed ? embed.embed.color : client.color.y)
                    .setAuthor({ name: embed.embed ? await variable(embed.embed.author.text, interaction) : null, iconURL: embed.embed ? await variable(embed.embed.author.icon, interaction) : null })
                    .setTitle(embed.embed ? await client.variable(embed.embed.title, interaction) : null)
                    .setDescription(embed.embed ? await client.variable(embed.embed.title, interaction) : null)
                    .setFooter({ text: embed.embed ? await variable(embed.embed.footer.text, interaction) : null, iconURL: embed.embed ? await variable(embed.embed.footer.icon, interaction) : null })
                    .setThumbnail(url ? await variable(url, interaction) : null)
                    .setImage(embed.embed ? await variable(embed.embed.image, interaction) : null)
                    .setTimestamp(embed.embed.timestamp === true ? new Date() : null)
            ]
        });
