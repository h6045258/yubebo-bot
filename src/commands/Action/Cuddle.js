...
module.exports = {
    ...
    cooldown: 3,
    description: {
        content: 'Hành động hoạt hình cuddle',
        example: 'cuddle @Yubabe bạn có thể nhập tên hoặc Id',
        usage: 'cuddle [@mention/username/userId]'
    },
    permission: {
        bot: [],
        user: []
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${message.member.displayName} tự âu yếm bản thân mình`,
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${user.displayName} đang âu yếm với ${user.displayName}`,
                            iconURL: message.member.displayAvatarURL({})
                        })
                        .setImage(await anime.cuddle())
                ]
