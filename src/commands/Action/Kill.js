...
module.exports = {
    ...
    cooldown: 3,
    description: {
        content: 'Hành động hoạt hình kill',
        example: 'kill @Yubabe bạn có thể nhập tên hoặc Id',
        usage: 'kill [@mention/username/userId]'
    },
    permission: {
        bot: [],
        user: []
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${message.member.displayName} tự kill chính mình`,
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${user.displayName} vừa mới kill ${user.displayName}`,
                            iconURL: message.member.displayAvatarURL({})
                        })
                        .setImage(await anime.kill())
                ]
