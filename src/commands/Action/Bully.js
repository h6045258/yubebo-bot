...
module.exports = {
    ...
    cooldown: 3,
    description: {
        content: 'Hành động hoạt hình bully',
        example: 'bully @Yubabe bạn có thể nhập tên hoặc Id',
        usage: 'bully [@mention/username/userId]'
    },
    permission: {
        bot: [],
        user: []
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${message.member.displayName} đang bắt nạt ai đó`,
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${user.displayName} vừa bắt nạt ${user.displayName}`,
                            iconURL: message.member.displayAvatarURL({})
                        })
                        .setImage(await anime.bully())
                ]
