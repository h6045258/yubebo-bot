...
module.exports = {
    ...
    cooldown: 3,
    description: {
        content: 'Hành động hoạt hình bite',
        example: 'bite @Yubabe bạn có thể nhập tên hoặc Id',
        usage: 'bite [@mention/username/userId]'
    },
    permission: {
        bot: [],
        user: []
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${message.member.displayName} tự cắn chính mình`,
            ...
            return message.channel.send({
                embeds: [
                    client.embed()
                        .setColor(client.color.y)
                        .setAuthor({
                            name: `${user.displayName} vừa bị ${user.displayName} cắn`,
                            iconURL: message.member.displayAvatarURL({})
                        })
                        .setImage(await anime.bite())
                ]
