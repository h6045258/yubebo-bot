...
const PRIMARY_CLUSTER_ID = 0; // Chỉ định cluster ID mà bạn muốn chạy tác vụ xổ số

/**
 * @param {import('discord.js').Client} client 
 */
module.exports = (client) => {
    client.isInLottery = new Collection();
    ...
    if (client.cluster.id == 0) {
        ...
        logger.info("[LOTTERY] BẮT ĐẦU SETUP XỔ SỐ")

        // CronJob.from({
        //     cronTime: "*/5 * * * *",
        //     onTick: () => {
        //         client.users.fetch("1216638785463390299").then((user) => user.send(`Send message from Cluser ID-${client.cluster.id}`));
        //     },
        //     start: true,
        //     timeZone: "Asia/Ho_Chi_Minh"
        // });
